...
const Movie = props => {
    ...
    const deleteReview = (reviewId, index) => {
         //Call the deleteReview method to remove the review by ID
        MovieDataService.deleteReview(reviewId, props.user.id)
            .then(response => {
                setMovie((currState) => {//Update the movie state to remove the deleted review
                    currState.reviews.splice(index, 1)//Remove the review from the reviews array using its index
                    return ({ //Return a new state object with the updated reviews
                        ...currState// Spread the previous state to retain other properties
                    })
                })
            })
            .catch(e => {
